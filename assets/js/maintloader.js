...
(function () {
	...
	function boot() {
		...
		if (!root || !window.MAINTLOADER) {
			return;
		}

		var CFG = window.MAINTLOADER;
		var state = {
			rows: [],
			hostids: [],
			sched: 'onetime',
		...
		function showTab(target) {
			all('.ml-tab').forEach(function (tab) {
				tab.classList.toggle('ml-tab-active', tab.getAttribute('data-tab') === target);
			});

			$('ml-panel-new').classList.toggle('ml-hidden', target !== 'new');
			$('ml-panel-active').classList.toggle('ml-hidden', target !== 'active');
		...
		function verify() {
			...
			return post('maintloader.verify', {hosts: input.value})
				.then(function (data) {
					if (data.error) {
		...
		$('ml-create').addEventListener('click', function () {
			...
			var action = 'maintloader.create';

			if (state.editing) {
				action = 'maintloader.update';
				payload.op = 'edit';
		...
		function loadActive() {
			...
			return post('maintloader.list', {show_expired: $('ml-show-expired').checked ? '1' : '0'})
				.then(function (data) {
					if (data.error) {
		...
		function startEdit(row) {
			return post('maintloader.update', {op: 'load', maintenanceid: row.maintenanceid})
				.then(function (data) {
					if (data.error) {
		...
		function renderActive(rows) {
			...
			$('ml-active-count').textContent = String(live);
			$('ml-active-count').className = 'ml-pill ' + (live ? 'ml-pill-good' : 'ml-pill-grey');
			...
			rows.forEach(function (row) {
				...
				if (row.editable) {
					var edit_btn = el('button', {type: 'button', class: 'ml-btn', text: 'Edit'});
				...
				end_btn.addEventListener('click', function () {
					...
					post('maintloader.end', {maintenanceid: row.maintenanceid})
						.then(function (data) {
							if (data.error) {
